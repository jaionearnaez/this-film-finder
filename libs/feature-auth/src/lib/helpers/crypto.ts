async function loadCrypto(): Promise<Crypto> {
  ...
  export async function encrypt(data: unknown, password: string): Promise<string> {
    ...
    const keyMaterial = await _crypto.subtle.importKey('raw', passwordBuffer, 'PBKDF2', false, [
      'deriveKey',
    ]);
    ...
    const ciphertext = await _crypto.subtle.encrypt({ name: 'AES-CBC', iv }, derivedKey, encodedData);
  ...
  export async function decrypt<ResponseType>(
    ...
    const _crypto = await loadCrypto();
    const encryptedArray = new Uint8Array(matches.map((byte) => parseInt(byte, 16)));
